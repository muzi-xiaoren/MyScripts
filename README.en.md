...
| [pt-checkin-mua](./pt-checkin-mua) | 2.0.0 | mua: on page open, auto-detect and do the daily check-in — if pending, go to the check-in page, clear the Cloudflare security check, check in and return to `special.php`; do nothing if already done | [Install](https://raw.githubusercontent.com/muzi-xiaoren/MyScripts/main/pt-checkin-mua/pt-checkin-mua.user.js) |
...
| [pt-checkin-dubhe](./pt-checkin-dubhe) | 1.0.0 | Dubhe: on page open, auto-detect and do the daily check-in — if pending, check in then return to `torrents.php?tag_id=3` and reload; do nothing if already done | [Install](https://raw.githubusercontent.com/muzi-xiaoren/MyScripts/main/pt-checkin-dubhe/pt-checkin-dubhe.user.js) |
| [pt-checkin-cspt](./pt-checkin-cspt) | 1.0.0 | CSPT: on page open, auto-detect and do the daily check-in (`not-attended` icon variant) — if pending, check in then return to `torrents.php` and reload; do nothing if already done | [Install](https://raw.githubusercontent.com/muzi-xiaoren/MyScripts/main/pt-checkin-cspt/pt-checkin-cspt.user.js) |
| [pt-checkin-hdfans](./pt-checkin-hdfans) | 1.0.0 | HDFans: on page open, auto-detect and do the daily check-in — if pending, check in then return to `torrents.php` and reload; do nothing if already done | [Install](https://raw.githubusercontent.com/muzi-xiaoren/MyScripts/main/pt-checkin-hdfans/pt-checkin-hdfans.user.js) |
| [pt-checkin-vclib](./pt-checkin-vclib) | 1.0.2 | VC-Lib: if today's check-in is pending, show the site's own captcha (2x scaled) in a centered panel on the page you're on — type all 6 characters and it checks in and reloads (you read the captcha; the script never solves it); nothing shows if already done | [Install](https://raw.githubusercontent.com/muzi-xiaoren/MyScripts/main/pt-checkin-vclib/pt-checkin-vclib.user.js) |
| [pt-checkin-bitporn](./pt-checkin-bitporn) | 1.0.0 | BitPorn (UNIT3D): on page open, claim today's reward from the current month's "Daily Reward" calendar (event id auto +1 per month), then return to `/trending`; do nothing if already claimed | [Install](https://raw.githubusercontent.com/muzi-xiaoren/MyScripts/main/pt-checkin-bitporn/pt-checkin-bitporn.user.js) |
| [pt-tangpt-helper](./pt-tangpt-helper) | 1.0.0 | tangpt multi-feature helper, one toggle per feature: (1) bulk inbox cleanup (delete every read message plus unread ones whose subject starts with 「任务」, keep the rest) (2) cumulative lottery log (side panel, persisted per day, bonus points tracked as totals only) | [Install](https://raw.githubusercontent.com/muzi-xiaoren/MyScripts/main/pt-tangpt-helper/pt-tangpt-helper.user.js) |
