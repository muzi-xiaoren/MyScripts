...
// @version      1.0.0
// @description  打开 Dubhe(天枢) 时自动检测签到状态：未签到就签到并回到指定页面，已签到则什么都不做。
// @author       muzi-xiaoren
// @match        https://dubhe.site/*
// @run-at       document-end
// @grant        none
// @homepageURL  https://github.com/muzi-xiaoren/MyScripts
// @supportURL   https://github.com/muzi-xiaoren/MyScripts/issues
// @downloadURL  https://raw.githubusercontent.com/muzi-xiaoren/MyScripts/main/pt-checkin-dubhe/pt-checkin-dubhe.user.js
// @updateURL    https://raw.githubusercontent.com/muzi-xiaoren/MyScripts/main/pt-checkin-dubhe/pt-checkin-dubhe.user.js
// @license      MIT
// ==/UserScript==
...
(function () {
  ...
  //   ② 经典 faqlink 变体（Mua/KamePT/NicePT/GGPT/CarPT/Dubhe/HDFans）：<a class="faqlink" href="attendance.php">；已签后 faqlink class 被清空，靠结构判断。
  //   ③ 带 type 参数变体（PTtime）：<a href="attendance.php?type=sign&uid=..">；已签变 type=list，靠 type=sign 判断。
  //   ④ BTSchool 每日签到插件：<a href="index.php?action=addbonus">每日签到</a>；已签后该链接消失。
  //   ⑤ not-attended 图标变体（CSPT/财神）：<a class="not-attended" href="attendance.php">；已签后 class 变 attended，靠 not-attended 判断。
  ...
  const notAttendedLink = document.querySelector('a.not-attended[href*="attendance.php"]');
  const endpoint = ajaxLink
    ...
    : notAttendedLink
    ? notAttendedLink.getAttribute('href')
    : null;
